...
if ARGV[0] != '-h' then
	userpassfile = File.open(ARGV[0])
	userpass = userpassfile.readlines.map(&:chomp)
	userpassfile.close

	puts "Username\t\t\tPassword\n----\t\t\t\t----"
	
	userpass.each do |user_pass|
		sleep 1
	...
	puts 'Pass a list of usernames and passwords to check. --> ruby AzureVUE.rb [emails:password.txt]'
end
